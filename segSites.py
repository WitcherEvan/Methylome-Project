...
def compare(line1,line2,minBases,methylRatio):
    if methylRatio == '':       # branch where no ratio is given
        if minBases == '':      # no arguments given, so use given methylation_call value
            return int(line2[6]) + int(line1[6])
        elif int(line2[5]) >= minBases and int(line1[5]) >= minBases: # minBases given. Both must meet minBases to qualify
            return int(line2[6]) + int(line1[6])
        else: # considered unmethylated for not meeting minBases
            return 0

    elif minBases == '':      # branch where methylRatio is given, but minBases is not
        if int(line1[4]) / int(line1[5]) >= methylRatio:   # Only methylRatio given. Both must qualify to be considered
            mc1 = 1
        else:
            mc1 = 0
    ...
    elif int(line2[5]) >= minBases and int(line1[5]) >= minBases:   # branch with methylRatio and minBases
        if int(line1[4]) / int(line1[5]) >= methylRatio:    # since both have enough bases, we can evaluate based on methylRatio
            mc1 = 1
        else:
            mc1 = 0
        if int(line2[4]) / int(line2[5]) >= methylRatio:
            ...
            while 1: # We leave this otherwise infinite loop by hitting an IndexError at the end of a file.
                if line1[0] == line2[0]: # chromosomes are the same
                    if  line1[1] == line2[1]: # Positions are matched.
                        if chromo != 'all':
                            if chromo == int(line1[0]):
                                if int(line1[1]) < minPos:
                                    line1 = next(f1).decode('utf-8').strip('\n').split('\t')
                                    continue
                                elif int(line1[1]) > maxPos:
                                    print('End of comparison '+seq1+' & '+seq2)
                                    print(str(round(time.time() - subStart, 2)) + 'seconds for a comparison.')
                                    break # from 'while 1', go to writing results
                            if chromo > int(line1[0]):
                                line1 = next(f1).decode('utf-8').strip('\n').split('\t')
                                continue
                            elif chromo < int(line1[0]): # past the designated chromose
                                print('End of comparison '+seq1+' & '+seq2)
                                print(str(round(time.time() - subStart, 2)) + 'seconds for a comparison.')
                                break # from 'while 1', go to writing results
# keep same 0,1,2 code for 'check' but new determination for what is considered methylated
                        check = compare(line1,line2,minBases,methylRatio)
